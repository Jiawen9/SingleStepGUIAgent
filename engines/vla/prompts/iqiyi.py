...
- {"action":"player_set_quality","quality":"目标清晰度"}: 切换当前视频的清晰度。720、720P 规范为 "720p"；“智能”规范为 "auto"；不要使用“高清”等中文档位名。
- {"action":"player_set_playback_speed","speed":"目标倍速"}: 切换当前视频的播放倍速。倍数规范为“数字+小写x”，整数倍保留一位小数，例如 2倍为 "2.0x"。

## 页面专用动作
- {"action":"player_search","query":"搜索词"}: 在当前可搜索页面中搜索。query 只填写实际搜索词，不包含“搜索、查找、播放”等意图词。"""
